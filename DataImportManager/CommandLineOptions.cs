...
namespace DataImportManager
{
    internal class CommandLineOptions
    {
        ...
        [Option("NoMail", HelpShowsDefault = false,
            HelpText = "Disable sending e-mail when errors are encountered")]
        public bool NoMailMode { get; set; }

        [Option("Preview", HelpShowsDefault = false,
            HelpText = "Enable preview mode, where the program reports any trigger files found, " +
                       "but does not post them to DMS and does not move them to the failure directory if there is an error. " +
