...
#End Region
	...
	Public Function ValidateXMLFile(ByVal xmlFilePath As String) As IXMLValidateStatus.XmlValidateStatus
        ...
        Catch Err As Exception
            m_logger.PostEntry("clsXMLTimeValidation.ValidateXMLFile(), error opening connection, " & Err.Message, ILogger.logMsgType.logError, LOG_LOCAL_ONLY)
        ...
        Catch Err As Exception
            m_logger.PostEntry("clsXMLTimeValidation.ValidateXMLFile(), Error calling PerformValidation, " & Err.Message, ILogger.logMsgType.logError, LOG_LOCAL_ONLY)
            Return IXMLValidateStatus.XmlValidateStatus.XML_VALIDATE_ENCOUNTERED_ERROR
        End Try
        ...
        Try
            CLoseConnection()
        Catch Err As Exception
            m_logger.PostEntry("clsXMLTimeValidation.ValidateXMLFile(), Error closing connection, " & Err.Message, ILogger.logMsgType.logError, LOG_LOCAL_ONLY)
            Return IXMLValidateStatus.XmlValidateStatus.XML_VALIDATE_ENCOUNTERED_ERROR
        End Try
    ...
    Private Function GetXMLParameters(ByVal xmlFilename As String) As IXMLValidateStatus.XmlValidateStatus
        ...
        Catch Err As Exception
            m_logger.PostEntry("clsXMLTimeValidation.GetXMLParameters(), Error reading XML File, " & Err.Message, ILogger.logMsgType.logError, LOG_LOCAL_ONLY)
            Return IXMLValidateStatus.XmlValidateStatus.XML_VALIDATE_ENCOUNTERED_ERROR
        End Try
    ...
    Private Function InstrumentWaitDelay(ByVal xmlFilename As String) As IXMLValidateStatus.XmlValidateStatus
        ...
        Try
            ...
            If dateNow < fileModDateDelay Then
                m_logger.PostEntry("clsXMLTimeValidation.InstrumentWaitDelay(), The dataset import is being delayed for XML File: " + xmlFilename, ILogger.logMsgType.logWarning, LOG_LOCAL_ONLY)
        ...
        Catch Err As Exception
            m_logger.PostEntry("clsXMLTimeValidation.InstrumentWaitDelay(), Error determining wait delay, " & Err.Message, ILogger.logMsgType.logError, LOG_LOCAL_ONLY)
            Return IXMLValidateStatus.XmlValidateStatus.XML_VALIDATE_ENCOUNTERED_ERROR
        End Try
    ...
    Private Function SetDbInstrumentParameters(ByVal insName As String) As IXMLValidateStatus.XmlValidateStatus
        ...
        Catch Err As Exception
            m_logger.PostEntry("clsXMLTimeValidation.SetDbInstrumentParameters(), Error retrieving source path and capture type for instrument '" & insName & "': " & Err.Message, ILogger.logMsgType.logError, LOG_LOCAL_ONLY)
            Return IXMLValidateStatus.XmlValidateStatus.XML_VALIDATE_ENCOUNTERED_ERROR
        End Try
    ...
    Private Function PerformValidation() As IXMLValidateStatus.XmlValidateStatus
        ...
        Try
            ...
            If m_capture_Type = "secfso" AndAlso Not Environment.MachineName.ToLower().StartsWith("monroe") Then
                ...
                Else
                    ...
                    Else
                        m_logger.PostEntry("You can diagnose the problem using this query: ", ILogger.logMsgType.logError, LOG_LOCAL_ONLY)
                    End If
                    ...
                    m_logger.PostEntry(
                     ...
                     m_ins_Name & "'", ILogger.logMsgType.logError, LOG_LOCAL_ONLY)
                        ...
                        If m_run_Finish_Utc <> New DateTime(1960, 1, 1) Then
                            ...
                            Else
                                m_logger.PostEntry(
                                 ...
                                 " minute tolerance)", ILogger.logMsgType.logError, LOG_DATABASE)
                                Return IXMLValidateStatus.XmlValidateStatus.XML_VALIDATE_FAILED
                            End If
                        End If
        ...
        Catch Err As Exception
            m_logger.PostEntry("clsXMLTimeValidation.GetInstrumentName(), Error reading XML File, current task: " & currentTask & "; " & Err.Message, ILogger.logMsgType.logError, LOG_LOCAL_ONLY)
    ...
    Protected Function GetRawDSType(ByVal InstFolder As String, ByVal DSName As String, ByRef MyName As String) As RawDSTypes
        ...
        If Not diSourceFolder.Exists Then
            Dim statusMsg = "Source folder not found for dataset " & DSName & ": [" & InstFolder & "]"
    ...
    Protected Function VerifyConstantFileSize(ByVal FileName As String, ByVal SleepIntervalSeconds As Integer, ByRef blnLogonFailure As Boolean) As Boolean
        ...
        Catch ex As Exception
            m_logger.PostEntry("Error accessing '" & FileName & "': " & ex.Message, ILogger.logMsgType.logWarning, LOG_LOCAL_ONLY)
